...
class Medoo
{
    ...
    protected function dataImplode($data, &$map, $conjunctor)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if (
                ...
                if (
                    is_int($key) &&
                    preg_match('/([a-zA-Z0-9_\.]+)\[(?<operator>\>|\>\=|\<|\<\=|\!|\=)\]([a-zA-Z0-9_\.]+)/i', $value,
                        $match)
                ) {
                    $wheres[] = $this->columnQuote($match[1]) . ' ' . $match['operator'] . ' ' . $this->columnQuote($match[3]);
                } else {
                    preg_match('/(#?)([a-zA-Z0-9_\.]+)(\[(?<operator>\>|\>\=|\<|\<\=|\!|\<\>|\>\<|\!?~)\])?/i', $key,
                        $match);
    ...
    protected function selectContext($table, &$map, $join, &$columns = null, $where = null, $column_fn = null)
    {
        ...
        if (
            ...
            foreach ($join as $sub_table => $relation) {
                preg_match('/(\[(?<join>\<|\>|\>\<|\<\>)\])?(?<table>[a-zA-Z0-9_]+)\s?(\((?<alias>[a-zA-Z0-9_]+)\))?/',
                    $sub_table, $match);
    ...
    public function insert($table, $datas)
    {
        ...
        return $this->exec('INSERT INTO ' . $this->tableQuote($table) . ' (' . implode(', ',
                $fields) . ') VALUES ' . implode(', ', $stack), $map);
    }
    ...
    public function update($table, $data, $where = null)
    {
        ...
        return $this->exec('UPDATE ' . $this->tableQuote($table) . ' SET ' . implode(', ',
                $fields) . $this->whereClause($where, $map), $map);
    }
    ...
    public function has($table, $join, $where = null)
    {
        ...
        $query = $this->exec('SELECT EXISTS(' . $this->selectContext($table, $map, $join, $column, $where, 1) . ')',
            $map);
